...
void page_frame_allocator_init(void *start, void *end)
{
    ...
    for (int i = 0; i < pf_count; ++i)
        pf_entries[i].order = 0, pf_entries[i].status = 0;
}
...
void *page_frame_allocation(uint32_t page_num)
{
    ...
    frame_entry_list_head *felhp = pf_freelists[alloc_page_order].next;
...
void *chunk_slot_allocation(uint32_t size)
{
    ...
    cslh = cs_freelists[size_idx].next;
...
void memory_reserve(void *start, void *end)
{
    ...
    {
        int idx = address2idx(tmp);
        pf_entries[address2idx(tmp)].status = ALLOCATED;
    //    uart_printf("Reserve page idx : %d, address : 0x%x\r\r\n", idx, idx2address(idx));
    }
}
void memory_init()
{
    ...
    memory_reserve(&_startup_allocator_start, &_startup_allocator_end);
